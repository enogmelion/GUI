...
NetworkSinkEditor::NetworkSinkEditor(GenericProcessor* parentNode, bool useDefaultParameterEditors = true)
...

{
	...
	activeButton = new UtilityButton("ON", Font("Small Text", 13, Font::plain));
...
NetworkSinkEditor::~NetworkSinkEditor()
{

}

void NetworkSinkEditor::updateSettings()
{


}

void NetworkSinkEditor::setDefaults()
{

}

void NetworkSinkEditor::comboBoxChanged(juce::ComboBox* c)
{
...
void NetworkSinkEditor::saveCustomParameters(XmlElement* xml)
{
...
void NetworkSinkEditor::loadCustomParameters(XmlElement* xml)
{
...
void NetworkSinkEditor::buttonEvent(Button* button)
{
...
void NetworkSinkEditor::labelTextChanged(Label* label)
{
	NetworkSinkNode* processor = (NetworkSinkNode*)getProcessor();
	...
	if (label == portValue)
	{
		...
		{
			//processor->getParent->sendActionMessage("Value out of range (0-10000 ms accepted)");
...
void NetworkSinkEditor::setPort(int post)
{
	NetworkSinkNode* processor = (NetworkSinkNode*)getProcessor();
	processor->setParameter(2, (float)post);
	portValue->setText(String(post), dontSendNotification);
	lastPortString = portValue->getText();
}

void NetworkSinkEditor::setActive(bool positive)
{
	NetworkSinkNode* processor = (NetworkSinkNode*)getProcessor();
	
	processor->setParameter(8, (float)((int)positive));
	
	//if (processor->getServerActive())
	//{
	//	processor->sendActionMessage("Server is On");
	//}
	//else if (!processor->getServerActive())
	//{
	//	processor->sendActionMessage("Server is Off");
	//}
}

int NetworkSinkEditor::getPort()
{
	Value val = portValue->getTextValue();
	return (int)(val.getValue());
}

bool NetworkSinkEditor::getActive()
{
